...
export class RoyaltyService {
    ...
    // Common get Calls
    getCallsData(params): Observable<any[]> {
        return this._httpClient.get<any[]>(environment.royalties, {
            params: params
        }).pipe(retry(3));
    };
    PostCallsData(payload) {
        const headers = { 'Authorization': `Bearer ${this._authService.accessToken}` };
        return this._httpClient.post(
            environment.royalties, payload, { headers });
    };
    PutCallsData(payload) {
        const headers = { 'Authorization': `Bearer ${this._authService.accessToken}` };

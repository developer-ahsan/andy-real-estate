...
export class QuoteDashboardDetailsComponent implements OnInit, OnDestroy {
  ...
  updateReorder(imprint) {
    ...
    this._RapidBuildService.UpdateSmartArtData(payload).pipe(takeUntil(this._unsubscribeAll)).subscribe(res => {
      this._RapidBuildService.snackBar('Reorder updated successfully');
  ...
  updateQuote() {
    ...
    this._RapidBuildService.UpdateSmartArtData(payload).pipe(takeUntil(this._unsubscribeAll)).subscribe(res => {
      this._RapidBuildService.snackBar('Quote information updated successfully');
  ...
  updateArtworkTags() {
    if (this.selectedMultipleTags) {
      if (this.selectedMultipleTags.length > 0) {
        ...
        this._RapidBuildService.snackBar('Please select atleast 1 tag');
      }
    } else {
      this._RapidBuildService.snackBar('Please select atleast 1 tag');
    }
  ...
  uploadProofMedia() {
    if (!this.imageValue) {
      this._RapidBuildService.snackBar('Please choose any image');
      return;
    }
    ...
    this._RapidBuildService.AddSmartArtData(payload).subscribe(res => {
